...
DOUBLE PRECISION :: perca,percanis,perchexa,perctetra,percortho,percmono,perctri
...
DOUBLE PRECISION, DIMENSION(:,:), ALLOCATABLE :: vertices,azimuthal,xyz,TIaxis 
...
IF(cartspher == 2) THEN
   ...
   !$omp parallel & 
   !$omp shared(Sav,mx1,mx3,mYY) &
   !$omp private(m,phi1,theta,phi2,acs,lr,cr) &    
   !$omp firstprivate(pi,marknum)
   !$omp do schedule(guided,8)
   DO m = 1,marknum

      !phi2 : rotate toward -Y axis
      !theta: rotate toward Z axis (negative rotation around the X-axis
      !as it is anticlock-wise when looking toward +X direction)
      !(Phi -> X; Theta -> -Y; Radial -> Z)
      !
      !       Z (R)    
      !         |   
      !         |  / -X 
...
IF(reflectmod /= 0) THEN
   ...
   acs = 0d0 
   acs(1,1) = 1d0; acs(2,2) = 1d0; acs(3,3) = 1d0 
   IF(ABS(reflectmod) == 1) acs(1,1) =-1 !Reflect with respect to plane normal to x1
   !change acs(2,2) because tensors are rotated in local coordinates
   IF(ABS(reflectmod) == 3) acs(2,2) =-1 !Reflect with respect to plane normal to x3
...
ALLOCATE(Rhoref(nx21),Vpref(nx21),Vsref(nx21),Savref(6,6,nx21))
...
IF(specfem3Dmod > 0) THEN
   ...
   ! Save D-REX_M model interpolated to the VIZTOMO grid
   filenamexmf = str2//'cij_model.xyz'

   OPEN(20,file=filenamexmf,status='replace')
   WRITE(20,'(a)') '#    NX      NY      NZ'
   WRITE(20,'(3i)'),nx11,nx31,nx21
   WRITE(20,'(a)') '#    Lon'
   WRITE(20,'(3000f17.7)'),X1
   WRITE(20,'(a)') '#    Lat'
   WRITE(20,'(3000f17.7)'),X3
   WRITE(20,'(a)') '#    Depth'
   WRITE(20,'(3000f17.7)'),depth
   ...
   DO i1=1,nx11
      ...
      !phi1 = rotate around Z axis by pi/2
      !Phi -> Y, Theta -> X, Radial -> Z
      !
      !       Z (R)    
      !         |   
      !         |  / -X 
      !         | /
      ! -Y______|/______ Y (Phi)
      !         /
      !        /   
      !       /   
      !      / X (Theta) 
      !
      phi2 = 0d0; theta = 0d0; phi1 = +pi*0.5d0  

      !Transform Euler angles into direction cosine matrix
      CALL rotmatrixZXZ(phi1,theta,phi2,acs)

      CALL rotvoigt(Savn(:,:,gi),acs,Savn0)
...
DEALLOCATE(Rhoref,Savref,Vpref,Vsref)
...
IF(azimod .NE. 0) THEN
   ...
   ALLOCATE(azimuthal(3,azinum),TIaxis(3,azinum),idx(azinum))

   azimuthal = 0d0 ; TIaxis = 0d0 ; idx = 0
   ...
   DO i1=1,nx11
      ...
      IF(Rhon(gi) > 0 .AND. fsen(gi) >= ln_fse_min) THEN
         ...
         !Find TI axis as most unique axis (see Browayes et al., 2004, GJI)
         CALL DECSYM(Savn(:,:,gi),1,perca,phi_a,gi,percanis,perchexa,perctetra,percortho,percmono,perctri)
         TIaxis(:,azinum) = perca*phi_a
         ...
         ELSE
            ...
            !This rotation is inverse to the one applied to the Lagrangian
            !particles (line 227) 
            phi1 = X1(i1) + pi*0.5d0; theta = X3(i3) ; phi2 = 0d0
            IF(yyy == 2) THEN
               phi1 = lr + pi*0.5d0 ; theta = cr
            END IF
            ...
            TIaxis(:,azinum) = 0d0
            DO j=1,3; DO k=1,3
               TIaxis(j,azinum) = TIaxis(j,azinum) + acs(j,k)*azi(k)
            END DO; END DO
...
IF(dimensions == 3 .AND. azimod > 0) THEN
   ...
   IF(cartspher == 2) THEN
      ...
      phi = pi/2d0 !parallel to x2 axis

      aziscalex1 = aziscalex1*deg2rad
      aziscalex3 = aziscalex3*deg2rad

      !Rotate first around Y by theta, then around Z by phi      
      phi1 = aziscalex1 ; theta = aziscalex3 ; phi2 = 0d0
      CALL rotmatrixZYZ(phi1,theta,phi2,acs)
   ...
   print *,'Save TIaxis orientation'
   print * 
   CALL loadsave_double(0,2,file_id,dum_int(1:2),H5T_NATIVE_DOUBLE,TIaxis,'TIaxis',1)
   DEALLOCATE(TIaxis)
   ...
   WRITE(19,'(a)') '          </Attribute>'
   WRITE(19,'(a)') ' '
   WRITE(19,'(a)') '          <Attribute Type="Vector" Center="Node" Name="TI axis">'
   WRITE(19,'(a,i0,a,a,a)') '             <DataItem Format="HDF" NumberType="Float" Precision="8" Dimensions="',azinum,' 3">azianis',dt_str4,'.h5:/TIaxis</DataItem>'
   WRITE(19,'(a)') '          </Attribute>'
   WRITE(19,'(a)') ' '
   WRITE(19,'(a)') '   </Grid>'
   WRITE(19,'(a)') ' '
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   INTEGER :: i1,i2,i3,i,j,k,m,gi,gijk,x,y,z,tid,yyy,yyn,yydum
