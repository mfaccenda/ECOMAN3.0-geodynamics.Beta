...
DEALLOCATE(mx1,mx2,mx3,mYY,rho,ln_fse,Sav,Fij,mtk,mpgpa,rocktype,Rotm,amin,amed,amax)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   IF(dimensions == 3) THEN 
      ...
      if(reflectmod == -3 .AND. n3first >= i3first) print *,'Reflection about Zmin is active. Change Zmin of the tomographi grid'  
      if(reflectmod ==  3 .AND. n3last  <= i3last ) print *,'Reflection about Zmax is active. Change Zmax of the tomographi grid'  
   END IF
   ...
   !Check if Rho, Cij are available
   CALL h5lexists_f(group_id, 'rho', link_exists,error)
   IF(link_exists == .TRUE.) THEN
      fsemod = 0
      WRITE(*,"(a,i5)")  ,' fsemod :      ',fsemod
   ELSE
      fsemod = 1
      WRITE(*,"(a,i5,a)"),' fsemod :      ',fsemod,' --> ONLY Fij available'
      TIaxismod = 0
      vpmaxmod = 0
      dvsmaxmod = 0
      spomod = 0
   ...
   ALLOCATE(rho(marknum0),mtk(marknum0),mpgpa(marknum0),X1save(21,marknum0),Sav(6,6,marknum0))

   CALL loadsave_double(0,1,group_id,marknum,H5T_NATIVE_DOUBLE,rho,'rho',0)

   !Check if P,T conditions available
   CALL h5lexists_f(group_id, 'mtk', link_exists,error)
   IF(link_exists == .TRUE.) THEN
      ptmod = 1
      WRITE(*,"(a,i5,a)"),' ptmod :       ',ptmod,' --> P-T conditions available from Thermo-Mechanical model'
   ELSE
      ptmod = 0
      WRITE(*,"(a,i5,a)"),' ptmod :       ',ptmod,' --> P-T conditions not available from Mechanical model'
   END IF
   write(*,*)

   IF(ptmod > 0) CALL loadsave_double(0,1,group_id,marknum,H5T_NATIVE_DOUBLE,mtk,'mtk',0)
   IF(ptmod > 0) CALL loadsave_double(0,1,group_id,marknum,H5T_NATIVE_DOUBLE,mpgpa,'mpgpa',0)

   !Sav(GPa)
   dum_int(1)=21
   dum_int(2)=marknum
   CALL loadsave_double(0,2,group_id,dum_int(1:2),H5T_NATIVE_DOUBLE,X1save,'Sav',0)

   DO m = 1, marknum

       !Save elastic tensor: only 21 elastic moduli
       Sav(1,1,m) = X1save(1,m)
       Sav(2,2,m) = X1save(2,m)
       Sav(3,3,m) = X1save(3,m)
       Sav(2,3,m) = X1save(4,m)
       Sav(3,2,m) = X1save(4,m)
       Sav(1,3,m) = X1save(5,m)
       Sav(3,1,m) = X1save(5,m)
       Sav(1,2,m) = X1save(6,m)
       Sav(2,1,m) = X1save(6,m)
       Sav(4,4,m) = X1save(7,m)
       Sav(5,5,m) = X1save(8,m)
       Sav(6,6,m) = X1save(9,m)
       Sav(1,4,m) = X1save(10,m)
       Sav(4,1,m) = X1save(10,m)
       Sav(2,5,m) = X1save(11,m)
       Sav(5,2,m) = X1save(11,m)
       Sav(3,6,m) = X1save(12,m)
       Sav(6,3,m) = X1save(12,m)
       Sav(3,4,m) = X1save(13,m)
       Sav(4,3,m) = X1save(13,m)
       Sav(1,5,m) = X1save(14,m)
       Sav(5,1,m) = X1save(14,m)
       Sav(2,6,m) = X1save(15,m)
       Sav(6,2,m) = X1save(15,m)
       Sav(2,4,m) = X1save(16,m)
       Sav(4,2,m) = X1save(16,m)
       Sav(3,5,m) = X1save(17,m)
       Sav(5,3,m) = X1save(17,m)
       Sav(1,6,m) = X1save(18,m)
       Sav(6,1,m) = X1save(18,m)
       Sav(5,6,m) = X1save(19,m)
       Sav(6,5,m) = X1save(19,m)
       Sav(4,6,m) = X1save(20,m)
       Sav(6,4,m) = X1save(20,m)
       Sav(4,5,m) = X1save(21,m)
       Sav(5,4,m) = X1save(21,m)
