...
!!! Read input values for SPO model       
   ...
   IF(Volsavemin < Volsavestp .OR. Volsavemax < Volsavemin) THEN
      IF(Volsavemin < Volsavestp) write(*,*) ' Volsavemin < Volsavestp '
      IF(Volsavemax < Volsavemin) write(*,*) ' Volsavemax < Volsavemin '
      stop
   END IF
...
!print *,tid,cyc_esa1,cyc_esa2,esa(1),esa(2)
   ...
   DO cyc=1,volnum
      ...
      IF(Volsave(cyc) > 0) THEN
         ...
         C11(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(1,1)
         C22(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(2,2)
         C33(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(3,3)
         C44(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(4,4)
         C55(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(5,5)
         C66(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(6,6)
         C12(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(1,2)
         C13(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(1,3)
         C23(nu,vcyc,cyc_esa1,cyc_esa2)= Cdem(2,3)
      END IF
...
20 END DO 
   ...
   DEALLOCATE(Vol,Volsavedb,C11,C22,C33,C44,C55,C66,C12,C13,C23,esa12,esa23,etacontr)
