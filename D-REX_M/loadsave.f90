 ...
 !! ---------------------------------------------------------------------------
 !!
 !!    Copyright (c) 2018-2026, Università di Padova, Manuele Faccenda
 !!    All rights reserved.
 !!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   DOUBLE PRECISION :: vxx,vyy,vzz,koef1,koef2,sII,ttime,rem
   DOUBLE PRECISION :: x1stp2,x2stp2,x3stp2,V1xyz(3),V2xyz(3)
...
!!! End Load input files in hdf5 format

!!! Load V,T,P,Fd fields into 4-D and 5-D arrays
   DO yy = 1, yinyang
   ...
   !$omp firstprivate(nx1,nx2,nx3,nodenum,yy,dimensions,ptmod,fractdislmod,loadmode)
   DO i3 = 1, nx3
      DO i2 = 1, nx2
         DO i1 = 1, nx1
         ...
         IF(fractdislmod > 0 .AND. loadmode > 1) THEN
            !sII = Fd0(i4)
            !if(sII>=1d6) Fd(yy,i1,i2,i3) = 1d0
            !if(sII< 1d6) Fd(yy,i1,i2,i3) = 0d0
   ...
   END DO
!!! End Load V,T,P,Fd fields into 4-D and 5-D arrays
   ...
   !Return after loading initial P-T fields only to setup initial rocktype
   IF(loadmode == 1) RETURN

!!! Set Ux = Ui for cartesian model grids
   IF(cartspher == 1) THEN
   ...
   !$omp firstprivate(nx1,nx2,nx3)
   DO i3 = 1, nx3
      DO i2 = 1, nx2
         DO i1 = 1, nx1
         
            Ux(1,:,i1,i2,i3) = Ui(1,:,i1,i2,i3)
   ...
   END IF
!!! End Set Ux = Ui for cartesian model grids

!!! 2D model
   IF(dimensions == 2) THEN

!!! Convert velocities from polar to cartesian coordinate system
   IF(cartspher == 2) THEN

   !$omp parallel do & 
   !$omp schedule(static) &
   !$omp shared(Ui,Ux) &
   !$omp private(i1,i2,Adummy2) &    
   !$omp firstprivate(nx1,nx2,X1)
   DO i2 = 1, nx2
      DO i1 = 1, nx1
   ...
   END IF
!!! End Convert velocities from polar to cartesian coordinate system

!!! Compute velocity gradient tensor Dij
   IF(loadmode > 0) THEN
...
!!! Reset velocity gradient tensor
   ...
   !$omp firstprivate(nx1,nx2,X1,X2,x1periodic,x2periodic,basicstag,cartspher,texmod)
   DO i2 = 1, nx2
      DO i1 = 1, nx1
   ...
   END IF
!!! End Compute velocity gradient tensor Dij
...
!!! Convert Vlong from m/s to rad/s for advection
   ...
   END IF
!!! End Convert Vlong from m/s to rad/s for advection
...
!!! If needed, convert Vcolat and Vphi of Yang grid to Yin grid coordinate system    
   if(1==0 .AND. yy == 2) then
...
!!! End If needed, convert Vcolat and Vphi of Yang grid to Yin grid coordinate system    

!!! Convert velocities from spherical to global Cartesian coordinates
   !$omp parallel do & 
   !$omp schedule(static) &
   !$omp shared(X1,X3,Ui,Ux) &
   !$omp private(i1,i2,i3,lr,cr,Adummy) &    
   !$omp firstprivate(nx1,nx2,nx3,yy)
   DO i3 = 1, nx3    
      DO i2 = 1, nx2    
         DO i1 = 1, nx1   
   ...
   END DO
!!! End Convert velocities from spherical to global Cartesian coordinates
...
!!! Reset velocity gradient tensor
   ...
   !!$omp parallel do & 
   !!$omp schedule(static) &
   !!$omp shared(X1,X2,X3,Ux,Dij) &
   !!$omp private(i1,i2,i3,dV,Adummy,V1xyz,V2xyz,lr,cr,rr,xx,yyy,zz,x2z2) &    
   !!$omp firstprivate(nx1,nx2,nx3,yy,x1stp,x2stp,x3stp,x1stp2,x2stp2,x3stp2) &
   !!$omp firstprivate(x1periodic,x2periodic,x3periodic,yinyang,basicstag,cartspher,texmod)
   DO i3 = 1, nx3    
      DO i2 = 1, nx2    
         DO i1 = 1, nx1   
            ...
            IF(basicstag == 2) THEN
               ...
               IF (i1 < nx1 .AND. i2 < nx2 .AND. i3 < nx3) THEN 
                  !dVx/dx
                  Dij(yy,1,1,i1+1,i2+1,i3+1) = (Ux(yy,1,i1+1,i2,i3)-Ux(yy,1,i1,i2,i3))/(X1(i1+1)-X1(i1))
                  !dVy/dy
                  Dij(yy,2,2,i1+1,i2+1,i3+1) = (Ux(yy,2,i1,i2+1,i3)-Ux(yy,2,i1,i2,i3))/(X2(i2+1)-X2(i2))
                  !dVz/dz
                  Dij(yy,3,3,i1+1,i2+1,i3+1) = (Ux(yy,3,i1,i2,i3+1)-Ux(yy,3,i1,i2,i3))/(X3(i3+1)-X3(i3))
                  !!! Incompressibility           
               ...
               IF (x1periodic > 0 .and. yinyang == 1 .and. i1 == 1 .and. i3 < nx3) THEN
                  Dij(yy,2,1,i1,i2,i3) = (Ux(yy,2,1,i2,i3)-Ux(yy,2,nx1-1,i2,i3))/(X1(i1+1)-X1(i1)+X1(nx1)-X1(nx1-1))*2d0
                  Dij(yy,2,1,nx1,i2,i3) = Dij(yy,2,1,i1,i2,i3)
               ELSE 
                  IF (i1 > 1 .AND. i1 < nx1 .AND. i3 < nx3) THEN
                     Dij(yy,2,1,i1,i2,i3) = (Ux(yy,2,i1,i2,i3)-Ux(yy,2,i1-1,i2,i3))/(X1(i1+1)-X1(i1-1))*2d0
                  END IF
               END IF
               !dVz/dx
               !At (0,+ystp/2,0)
               IF (x1periodic > 0 .and. yinyang == 1 .and. i1 == 1 .and. i2 < nx2) THEN
                  Dij(yy,3,1,i1,i2,i3) = (Ux(yy,3,1,i2,i3)-Ux(yy,3,nx1-1,i2,i3))/(X1(i1+1)-X1(i1)+X1(nx1)-X1(nx1-1))*2d0
                  Dij(yy,3,1,nx1,i2,i3) = Dij(yy,3,1,i1,i2,i3)
               ELSE 
                  IF (i1 > 1 .AND. i1 < nx1 .AND. i2 < nx2) THEN
                     Dij(yy,3,1,i1,i2,i3) = (Ux(yy,3,i1,i2,i3)-Ux(yy,3,i1-1,i2,i3))/(X1(i1+1)-X1(i1-1))*2d0
                  END IF
               END IF
               ...
               IF (x2periodic > 0 .and. i2 == 1 .and. i3 < nx3) THEN
                  Dij(yy,1,2,i1,i2,i3) = (Ux(yy,1,i1,1,i3)-Ux(yy,1,i1,nx2-1,i3))/(X2(i2+1)-X2(i2)+X2(nx2)-X2(nx2-1))*2d0
                  Dij(yy,1,2,i1,nx2,i3) = Dij(yy,1,2,i1,i2,i3)
               ELSE 
                  IF (i2 > 1 .AND. i2 < nx2 .AND. i3 < nx3) THEN
                     Dij(yy,1,2,i1,i2,i3) = (Ux(yy,1,i1,i2,i3)-Ux(yy,1,i1,i2-1,i3))/(X2(i2+1)-X2(i2-1))*2d0
                  END IF
               END IF
               ...
               IF (x2periodic > 0 .and. i2 == 1 .and. i1 < nx1) THEN
                  Dij(yy,3,2,i1,i2,i3) = (Ux(yy,3,i1,1,i3)-Ux(yy,3,i1,nx2-1,i3))/(X2(i2+1)-X2(i2)+X2(nx2)-X2(nx2-1))*2d0
                  Dij(yy,3,2,i1,nx2,i3) = Dij(yy,3,2,i1,i2,i3)
               ELSE 
                  IF (i2 > 1 .AND. i2 < nx2 .AND. i1 < nx1) THEN
                     Dij(yy,3,2,i1,i2,i3) = (Ux(yy,3,i1,i2,i3)-Ux(yy,3,i1,i2-1,i3))/(X2(i2+1)-X2(i2-1))*2d0
                  END IF
               END IF
               ...
               IF (x3periodic > 0 .and. yinyang == 1 .and. i3 == 1 .and. i2 < nx2) THEN
                  Dij(yy,1,3,i1,i2,i3) = (Ux(yy,1,i1,i2,1)-Ux(yy,1,i1,i2,nx3-1))/(X3(i3+1)-X3(i3)+X3(nx3)-X3(nx3-1))*2d0
                  Dij(yy,1,3,i1,i2,nx3) = Dij(yy,1,3,i1,i2,i3)
               ELSE 
                  IF (i3 > 1 .AND. i3 < nx3 .AND. i2 < nx2) THEN
                     Dij(yy,1,3,i1,i2,i3) = (Ux(yy,1,i1,i2,i3)-Ux(yy,1,i1,i2,i3-1))/(X3(i3+1)-X3(i3-1))*2d0
                  END IF
               END IF
               ...
               IF (x3periodic > 0 .and. yinyang == 1 .and. i3 == 1 .and. i1 < nx1) THEN
                  Dij(yy,2,3,i1,i2,i3) = (Ux(yy,2,i1,i2,1)-Ux(yy,2,i1,i2,nx3-1))/(X3(i3+1)-X3(i3)+X3(nx3)-X3(nx3-1))*2d0
                  Dij(yy,2,3,i1,i2,nx3) = Dij(yy,2,3,i1,i2,i3)
               ELSE 
                  IF (i3 > 1 .AND. i3 < nx3 .AND. i1 .LT. nx1) THEN
                     Dij(yy,2,3,i1,i2,i3) = (Ux(yy,2,i1,i2,i3)-Ux(yy,2,i1,i2,i3-1))/(X3(i3+1)-X3(i3-1))*2d0
                  END IF
               END IF
            ...
            ELSE
                ...
                V1xyz = 0d0 ; V2xyz = 0d0
                IF(i1 > 1 .AND. i1 < nx1) THEN
                   V1xyz = Ux(yy,:,i1-1,i2,i3)
                   V2xyz = Ux(yy,:,i1+1,i2,i3)
                END IF
                IF(yinyang == 2 .AND. i1 == 1) THEN
                   CALL interpUx(X1(i1)-x1stp,i2,X3(i3),yy,V1xyz)
                   V2xyz = Ux(yy,:,2,i2,i3)
                END IF
                IF(yinyang == 2 .AND. i1 == nx1) THEN
                   V1xyz = Ux(yy,:,nx1-1,i2,i3)
                   CALL interpUx(X1(i1)+x1stp,i2,X3(i3),yy,V2xyz)
                END IF
                ...
                !dVx/dPhi  
                dV(1,1) = (V2xyz(1) - V1xyz(1))/x1stp2
                !dVy/dPhi  
                dV(2,1) = (V2xyz(2) - V1xyz(2))/x1stp2
                !dVz/dPhi  
                dV(3,1) = (V2xyz(3) - V1xyz(3))/x1stp2


                !dVi/dR   
                V1xyz = 0d0 ; V2xyz = 0d0
                IF(i2 > 1 .AND. i2 < nx2) THEN
                   V1xyz = Ux(yy,:,i1,i2-1,i3)
                   V2xyz = Ux(yy,:,i1,i2+1,i3)
                END IF
                IF(cartspher == 1 .AND. x2periodic > 0 .AND. (i2 == 1 .OR. i2 == nx2)) THEN
                   V1xyz = Ux(yy,:,i1,nx2-1,i3)
                   V2xyz = Ux(yy,:,i1,2    ,i3)
                END IF

                !dVx/dR    
                dV(1,2) = (V2xyz(1) - V1xyz(1))/x2stp2
                !dVy/dR    
                dV(2,2) = (V2xyz(2) - V1xyz(2))/x2stp2
                !dVz/dR    
                dV(3,2) = (V2xyz(3) - V1xyz(3))/x2stp2
                ...
                IF(i3 > 1 .AND. i3 < nx3) THEN
                   V1xyz = Ux(yy,:,i1,i2,i3-1)
                   V2xyz = Ux(yy,:,i1,i2,i3+1)
                END IF
                IF(yinyang == 2 .AND. i3 == 1) THEN
                   CALL interpUx(X1(i1),i2,X3(i3)-x3stp,yy,V1xyz)
                   V2xyz = Ux(yy,:,i1,i2,2)
                END IF
                IF(yinyang == 2 .AND. i3 == nx3) THEN
                   V1xyz = Ux(yy,:,i1,i2,nx3-1)
                   CALL interpUx(X1(i1),i2,X3(i3)+x3stp,yy,V2xyz)
                END IF
                ...
                !dVx/dTheta
                dV(1,3) = (V2xyz(1) - V1xyz(1))/x3stp2
                !dVy/dTheta
                dV(2,3) = (V2xyz(2) - V1xyz(2))/x3stp2
                !dVz/dTheta
                dV(3,3) = (V2xyz(3) - V1xyz(3))/x3stp2
                ...
                ELSE
                   ...
                   IF(isnan(Dij(yy,1,1,i1,i2,i3))) THEN
                      print *,'Dij is NaN'
                      print *,yy,i1,i2,i3
                      print *,Dij(yy,:,:,i1,i2,i3)
                      print *,dV
                      print *,Adummy
                   END IF
                ...
                !Incompressibility
                IF(texmod > 0) Dij(yy,3,3,i1,i2,i3) = -Dij(yy,1,1,i1,i2,i3)-Dij(yy,2,2,i1,i2,i3)
   ...
   END IF
!!! End computing velocity gradient tensor Dij
...
!!! Convert Vcolat and Vphi of Yang grid from Yin to Yang grid coordinate system    
            ...
            DO i3 = 1, nx3    
               DO i2 = 1, nx2    
                  DO i1 = 1, nx1   
                     CALL yin2yang(X1(i1),X3(i3),lr,cr)
                     koef1 = -sin(X1(i1))*sin(lr)
                     koef2 =  cos(lr)/sin(X3(i3))
                     if(isnan(koef2) .OR. cos(X1(i1)) == 0d0 .OR. ABS(sin(cr))<1.0d-10) koef2 = 0d0
                     vxx = Ui(yy,1,i1,i2,i3)
                     vzz = Ui(yy,3,i1,i2,i3)
                     Ui(yy,1,i1,i2,i3) = koef1*vxx + koef2*vzz
                     Ui(yy,3,i1,i2,i3) = koef1*vzz - koef2*vxx
         ...
         END IF
!!! End Convert Vcolat and Vphi of Yang grid from Yin to Yang grid coordinate system    

!!! Convert Vcolat and Vphi from m/s to rad/s for advection
         !$omp parallel do & 
         !$omp schedule(static) &
         !$omp shared(X1,X2,X3,Ui) &
         !$omp private(i1,i2,i3,lr,cr) &    
         !$omp firstprivate(nx1,nx2,nx3,yy,pi)
         DO i3 = 1, nx3    
            DO i2 = 1, nx2    
               DO i1 = 1, nx1   
         ...
         END DO
         !$omp end parallel do
!!! End Convert Vcolat and Vphi from m/s to rad/s for advection
...
!!! Compute true timestep and number of cycles for the current velocity field

   IF(Tinit <  Tend) ttime = dt0     
   IF(Tinit == Tend) ttime = timemax

   numdt = FLOOR(ttime/dt)
   rem =MOD(ttime,dt)
   IF(rem > 0) THEN
      numdt = numdt + 1
      dt = ttime /REAL(numdt)
   ...
   if ( rankMPI .eq. 1 ) then
      write(*,'(a,1es13.6)') ' TOTAL TIMESTEP     = ',ttime
      write(*,*)
      write(*,'(a,1es13.6,a,i13)') ' ADVECTION TIMESTEP = ',dt,', NUMBER OF CYCLES = ',numdt
      write(*,*)
   endif

!!! End Compute true timestep and number of cycles for the current velocity field
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! subroutine interpUx, interpolate cartesian velocity from other grid     !!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !Vx
   y1 = Ux(yyy,1,i1s,i2s,i3s) ;     y3 = Ux(yyy,1,i1s+1,i2s,i3s)
   y2 = Ux(yyy,1,i1s,i2s,i3s+1) ;   y4 = Ux(yyy,1,i1s+1,i2s,i3s+1)

   CALL interp2DUx(lr1,cr1,i1s,i3s,y1,y2,y3,y4,Uxx(1))

   !Vy
   y1 = Ux(yyy,2,i1s,i2s,i3s) ;     y3 = Ux(yyy,2,i1s+1,i2s,i3s)
   y2 = Ux(yyy,2,i1s,i2s,i3s+1) ;   y4 = Ux(yyy,2,i1s+1,i2s,i3s+1)

   CALL interp2DUx(lr1,cr1,i1s,i3s,y1,y2,y3,y4,Uxx(2))

   !Vz
   y1 = Ux(yyy,3,i1s,i2s,i3s) ;     y3 = Ux(yyy,3,i1s+1,i2s,i3s)
   y2 = Ux(yyy,3,i1s,i2s,i3s+1) ;   y4 = Ux(yyy,3,i1s+1,i2s,i3s+1)

   CALL interp2DUx(lr1,cr1,i1s,i3s,y1,y2,y3,y4,Uxx(3))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE interp2DUx(X1s,X3s,i1s,i3s,y1,y2,y3,y4,res)
   ...
   DOUBLE PRECISION :: y1,y2,y3,y4,xx,zz
...
!M3E!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   IF(fsemod == 0) THEN
      ...
      mtk = 300.0d0 ; mpgpa = 1.0d5

      !$omp parallel do & 
      !$omp schedule(dynamic) &
      !$omp shared(mx1,mx2,mx3,mYY,rocktype,Xol,odf,odf_ens,acs,acs_ens,acs0,Fij,Tk,Pa,Fd,td_rho,rho) &
      !$omp shared(mtk,mpgpa,mdb,Sav,S0,dS0dp,dS0dp2,dS0dt,dS0dpt,dS0dp5,dS0dt5,ijkl,mandel_scale,l1,l2) &
      !$omp private(m) &    
      !$omp firstprivate(marknum,ptmod,uppermantlemod,texmod,nboxnum,size3)
      DO m = 1 , marknum
   ...
   if (rankMPI .eq. 1) then
      write(*,*)
      write(*,'(a,1f10.2,a)') ' CALCULATE ELASTIC TENSOR OK! (',wtime,' sec)'
      write(*,*)
   endif
...
!M3E!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   INTEGER :: j,m,m1,t,nx(3),ncyc2,dum_int(3)
...
!M3E!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !call h5pset_dxpl_mpio_f(plistH5,H5FD_MPIO_INDEPENDENT_F,errH5)
