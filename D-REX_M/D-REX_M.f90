 ...
 !! ---------------------------------------------------------------------------
   ...
   INTEGER :: m,t,ncyc,ncyc1,ncyc2,ndt,nfile !! loop counters
...
!!! First advect backward and save position and rocktype
...
!!! Load file with velocity,P,T,Fd fields
...
15 ncyc  = ncyc  + 1 !Update number of cycles since the start of the forward run                       
   ...
   ndt = ndt + 1
   !IF(ncyc1 == 1 .AND. Tinit .NE. Tend) timesum = timesum - dt0
   timesum = timesum + dt
   IF(ndt == numdt .AND. Tinit == Tend) timesum = timemax
...
!!! Steady-state flow (works when Tinit = Tend and timemax > 0 in input.dat)
   ...
   !$omp shared(mx1,mx2,mx123,mrtYY,X1,X2,Dij,Fd,Fij,e,l,epsnot,odf,odf_ens,acs0,acs,acs_ens,rocktype,rho,fractdislrock,max_strain) &
   !$omp private(tid,m,i1,i2,fractdisl) &    
   !$omp firstprivate(marknum,dt,size,size3,alt,lambda,Xol,Mob,chi,tau,stressexp,strainmax,ncyc) &
...
!!! Aggregate transformation
   ...
   !$omp parallel do & 
   !$omp schedule(dynamic) &
   !$omp shared(mx1,mx2,mx3,mx123,mrtYY,mYY,X1,X2,X3,Dij,Ui,Fd,epsnot,rocktype,max_strain) &
...
!!! Check number of markers with max_strain >= strainmax 
   ...
   END DO
   call MPI_Allreduce(numstrainmax, numstrainmax_sum, 1, MPI_INTEGER, MPI_SUM, MPI_COMM_WORLD, errMPI)

   if ( rankMPI .eq. 1 ) then
      write(*,'(a,1i)') '    Total number of aggregates that have reached strainmax = ',numstrainmax_sum
      write(*,*)
   endif

!!! End backward run if all markers have cumulated enough deformation
   IF(numstrainmax_sum == marknumtot) THEN
      Tinit = t
      GOTO 90
   END IF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   INTEGER :: i1,i2,m,tid,ncyc,rocktype_old
   ...
   !$omp private(tid,m,i1,i2,fractdisl,rocktype_old) &    
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   INTEGER :: i1,i2,i3,m,tid,ncyc,rocktype_old
   DOUBLE PRECISION :: wtime,fractdisl
   !M3E!!!!!!!!!!!!!!!!!!!!!
   integer :: errMPI,rankMPI
   !M3E!!!!!!!!!!!!!!!!!!!!!
   ...
   !$omp shared(odf,odf_ens,acs,acs_ens,rocktype,rho,fractdislrock) &
   !$omp private(tid,m,i1,i2,i3,fractdisl,rocktype_old) &    
