...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   INTEGER, intent(in) :: m,tid
   DOUBLE PRECISION, intent(in) :: fractdisl

   INTEGER :: j,j1,j2,nnum,N_strain
   ! loop counters

   DOUBLE PRECISION :: dt_strain,dt_straintot,dt_straindiff
   ...
   DOUBLE PRECISION, DIMENSION(nboxnum) :: dotodf,dotodf_ens,odfi,odfi_ens
   DOUBLE PRECISION, DIMENSION(nboxnum) :: kodf1,kodf2,kodf3,kodf4
   DOUBLE PRECISION, DIMENSION(nboxnum) :: kodf1_ens,kodf2_ens,kodf3_ens,kodf4_ens
   ! intermediates for odf

   DOUBLE PRECISION, DIMENSION(nboxnum,3,3) :: dotacs,dotacs_ens,acsi,acsi_ens
   DOUBLE PRECISION, DIMENSION(nboxnum,3,3) :: kac1,kac2,kac3,kac4
   DOUBLE PRECISION, DIMENSION(nboxnum,3,3) :: kac1_ens,kac2_ens,kac3_ens,kac4_ens
   ! intermediates for matrix of direction cosine

!!! ODFTEX
   IF(texmod>0) CALL ODFDRX(0,tid,m,fractdisl,odfi,dotodf,acsi,dotacs)
...
!!! Compute fabric for both phases for upper mantle aggregates   
   ...
   IF(texmod==0 .AND. fsemod == 0 .AND. rocktype(m) /= 3) THEN
...
!!! Strain-induced LPO when dislocation creep is active           
   IF(fractdisl > 0d0) THEN
      ...
      DO j = 1 , size ; DO j2 = 1 , 3 ; DO j1 = 1, 3
