...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   DOUBLE PRECISION :: mtk,mpgpa,volfract,fintegral
   ...
   DOUBLE PRECISION, DIMENSION(nboxnum,3,3) :: acs_ol,acs_opx

   volfract = 1d0/REAL(nboxnum)      

   !Run ODFTEX
10 IF(texmod > 0 .AND. rocktype(m) == 1 ) CALL ODFDRX(1,0,m,1,f1_ol,f1_opx,acs_ol,acs_opx)

   C0 = 0d0 ; Cav = 0d0 ; CMP = 0d0 ; CMP0 = 0d0 ; CMP2 = 0d0 
   ...
   IF(texmod > 0) THEN
      ...
      IF(rocktype(m) == 1) THEN
         DO nu = 1 , nboxnum
            IF(fractvoigt > 0d0) CALL Caverage(f1_ol(nu)*Xol(rocktype(m)),acs_ol(nu,:,:),C0,Cav)
            IF(fractvoigt < 1d0) CALL Caverage(f1_ol(nu)*Xol(rocktype(m)),acs_ol(nu,:,:),CMP0,CMP) 
         END DO
      ELSE
         IF(ptmod > 0) THEN
            !Find isotropic tensor of rock from TD database               
            CALL isotensor(m,mtk,mpgpa,1,Sav1)
            RETURN 
         ELSE
            !Find isotropic tensor of major phase and add to total tensor
            CALL c2sav(C0,Sav1)
   ...
   IF(texmod > 0) THEN
      ...
      IF(rocktype(m) == 1 ) THEN
         ...
         DO nu = 1 , nboxnum
            IF(fractvoigt > 0d0) CALL Caverage(f1_opx(nu)*(1.0-Xol(rocktype(m))),acs_opx(nu,:,:),C0,Cav)
            IF(fractvoigt < 1d0) CALL Caverage(f1_opx(nu)*(1.0-Xol(rocktype(m))),acs_opx(nu,:,:),CMP0,CMP)
         END DO

      ELSE
         !Find isotropic tensor of minor phase and add to total tensor
         CALL c2sav(C0,Sav1)
         CALL isotensor(m,mtk,mpgpa,0,Sav1)
   ...
   IF(isnan(Cav(1,1,1,1)) .OR. (Cav(1,1,1,1)>400d0 .AND. rocktype(m)==1)) THEN
      IF(isnan(Cav(1,1,1,1))) print *,"Cav Nan at"
      IF(Cav(1,1,1,1)>400d0 .AND. rocktype(m)==1) print *,"Cav too large at"

      IF(texmod == 0 ) THEN
         write(*,'(2i,8f17.6)') m,rocktype(m),mx1(m),mx2(m),mx3(m),mtk,mpgpa,odf(m,1),acs(1,1,1,m),acs_ens(1,1,1,m),Cav(1,1,1,1)
         acs(:,:,:,m) = acs0
         acs_ens(:,:,:,m) = acs0
         odf(m,:) = volfract           
         odf_ens(m,:) = odf(m,:)
      ELSE
         !IF(rocktype(m)==1) THEN
         write(*,'(2i,8f17.6)') m,rocktype(m),mx1(m),mx2(m),mx3(m),mtk,mpgpa,sum(f1_ol),sum(f1_opx),Cav(1,1,1,1)
         !  Normalize integral of ODF to unity
         CALL INTEGRAL_DRX(fm(m,:,:,:),fm_nodrx(m,:,:,:),fintegral)
         write(6,*) 'fintegral =', fintegral, m
         pham(m,:,:,:) = pha0
         tham(m,:,:,:) = tha0
         psam(m,:,:,:) = psa0
           fm(m,:,:,:) = f0   
...
!       ----- +X
   ...
   IF(mtk<300.0d0) mtk=300.0d0
   IF(mpa<1.0d5) mpa=1.0d5
   ...
   G1=td_vs(db,n1  ,n2+1)
   G2=td_vs(db,n1+1,n2  )
   G3=td_vs(db,n1+1,n2+1)
   K0=td_vp(db,n1  ,n2  )
   K1=td_vp(db,n1  ,n2+1)
   K2=td_vp(db,n1+1,n2  )
   K3=td_vp(db,n1+1,n2+1)
   
   !Vs (km/s)
   Vs = ((G0*(1.0-n)+G1*n)*(1.0-ee)+(G2*(1.0-n)+G3*n)*ee)
   !Swave modulus (GPa)
   G = rho(m)*(Vs)**2.0d0/1.0d3

   !Vp (km/s)
   Vp = ((K0*(1.0-n)+K1*n)*(1.0-ee)+(K2*(1.0-n)+K3*n)*ee)
   !Pwave modulus (GPa)
   K = rho(m)*(Vp)**2.0d0/1.0d3 
